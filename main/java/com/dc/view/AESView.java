...
public class AESView extends JDialog {
    ...
    private JButton 发送文件Button;
    private JButton 接收文件Button;
    private JTextField 发送TxtTextField;
    private JTextField 接收TxtTextField;
    ...
    private static RecieveFileThread rft;
    ...
    public AESView() {
        ...
        发送文件Button.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                try {
                    //                位数
                    ...
                    String text = textField5.getText();
                    //              明文
                    FileInputStream fileInputStream = new FileInputStream(new File(发送TxtTextField.getText()));
                    ...
                    long startTime = System.nanoTime();
                    bytes1 = AESUtil.encryptOrdecrypt(true, context.getBytes("UTF-8"), text, bytes, selectedItem, type);
                    long endTime = System.nanoTime(); //获取结束时间
                    String s = Base64.getEncoder().encodeToString(bytes1);
                    //向局域网发送密文
                    if (!对方IP.getText().equals("")) {
        ...
        接收文件Button.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                接收文件Button.setText("正在监听");
                发送文件Button.setEnabled(false);
                发送文件Button.setEnabled(false);
                接收Button.setEnabled(false);
                发送Button.setEnabled(false);
                rft = new RecieveFileThread();
                rft.start();
            }
        });
    ...
    class RecieveThread extends Thread {
        public void run() {
            try {
                ...
                接收文件Button.setEnabled(true);
                发送文件Button.setEnabled(true);
            } catch (IOException e) {
    ...
    class RecieveFileThread extends Thread {
        public void run() {
            try {
                ServerSocket ss = new ServerSocket(10010);
                传输数据textArea.setText("文件监听端口等待数据......");
                Socket s = ss.accept();
                BufferedReader br = new BufferedReader(new InputStreamReader(s.getInputStream()));
                //读取客户端发送来的消息
                String mess = br.readLine();
                ...
                try {
                    ...
                    byte[] bytes1 = AESUtil.encryptOrdecrypt(false, text1, text, bytes, selectedItem, type);
                    String str = new String(bytes1);
                    FileOutputStream fileOutputStream = new FileOutputStream(new File(接收TxtTextField.getText()));
                    fileOutputStream.write(bytes1);
                    传输数据textArea.setText("接收到加密文件，存储到"+接收TxtTextField.getText());
